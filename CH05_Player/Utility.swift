...
class Utility {
    ...
    static func applyEncoderCookie(fromQueue queue: AudioQueueRef, toFile file: AudioFileID) {
        ...
        let result = AudioQueueGetPropertySize(queue, kAudioConverterCompressionMagicCookie, &propertySize)
        ...
        if result == noErr && propertySize > 0 {
            ...
            // get the cookie
            check(error: AudioQueueGetProperty(queue,
                                               kAudioQueueProperty_MagicCookie,
                                               magicCookie, &propertySize),
                  operation: "get audio queue's magic cookie")
    
            // now set the magic cookie on the output file
            check(error: AudioFileSetProperty(file,
                                              kAudioFilePropertyMagicCookieData,
                                              propertySize, magicCookie),
                  operation: "set audio file's magic cookie")
    ...
    static func applyEncoderCookie(fromFile file: AudioFileID, toQueue queue: AudioQueueRef) {
        ...
        if result == noErr && propertySize > 0 {
            ...
            // YES, allocate space for it
            let magicCookie: UnsafeMutablePointer<Void>  = malloc(4)
